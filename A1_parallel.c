...
// A function to implement the Boyer-Moore algorithm
...
//   printf("%lu",sizeof(pat_2d_array));
    ...
    #pragma omp parallel shared(pat_2d_array, txt_1d_array, size) //private (randomSeed)
    {
        //randomSeed = omp_get_thread_num() * time(NULL);
        clock_gettime(CLOCK_MONOTONIC, &startComp); 

        //For example, if the iterations have different execution times, 
        //then using smaller chunks can help distribute the work more evenly 
        //among the threads, as each thread can request a new chunk when it 
        //finishes its previous one. However, this also introduces some overhead 
        //for managing the shared queue of chunks. On the other hand, if the iterations 
        //have similar execution times, then using larger chunks can reduce the overhead 
        //and improve the cache locality, as each thread can work on a contiguous block of iterations.
        #pragma omp for schedule(dynamic, CHUNK_SIZE) reduction(+:size)
...
// A function to test the functions above
void test() {
    char *t = "fefefafe";
    char *p = "fe";
    int *size;
    *size = 0;
    ...
    int **table = bad_chara(t);
    printf("Bad character table:\n");
    print_2d_array(table, 26, strlen(t));
    
    int *z = z_array(t);
    printf("Z-array:\n");
    print_1d_array(z, strlen(t));

    int *zr = z_rev(t);
    printf("Reversed Z-array:\n");
    print_1d_array(zr, strlen(t));

    int *gs = good_suffix(t);
    printf("Good suffix array:\n");
    print_1d_array(gs, strlen(t) + 1);
    
    int *mp = match_prefix(t);
    printf("Match prefix array:\n");
    print_1d_array(mp, strlen(t) + 1);
    
    int result = boyer_moore(t, p);
    //printf("%d", *size);
    // printf("Pattern indices:\n");
    
    // printf("%lu", sizeof(result));
    // printf("\n");
    // print_1d_array(result, sizeof(result)); // Size is the global variable for the size of the dynamic array
    
    free_2d_array(table, 26); // Free memory allocated for the table
    free(gs); // Free memory allocated for the good suffix array
    free(mp); // Free memory allocated for the match prefix array
    free(z); // Free memory allocated for the Z-array
    free(zr); // Free memory allocated for the reversed Z-array
    //free(result); // Free memory allocated for the pattern indices
}
